...
def load_frames(path, format="npz"):
    """
    Load frames from the given path to npz file.

    Args:
        npz_path (str): the path to npz file.
    Returns:
        frames (Tensor): the loaded frames.
    """
    if format == "jpg":
        imgs = []
        for image_path in path:
            with PathManager.open(image_path, "rb") as f:
                ...
                img = cv2.imdecode(img_str, flags=cv2.IMREAD_COLOR)
            imgs.append(img)
        frames = torch.as_tensor(np.stack(imgs))
        # OpenCV uses BGR order of channels.
        frames = torch.index_select(frames, dim=3, index=torch.Tensor([2, 1, 0]).long())
    elif format == "npz" or "npz_compressed":
        with PathManager.open(path, "rb") as f:
            #print('path: ', path)
            frames = torch.as_tensor(np.load(path)['frames'])
    else:
        raise NotImplementedError()
    return frames


def save_frames(path, frames: torch.Tensor, format="npz"):
    """
    Save the frames to the given npz file path.

    Args:
        npz_path (list): save path for npz file.
        frames (Tensor): stack tensor of frames, need to be compatible with OpenCV.
    """
    if format == 'jpg':
        if not len(frames) == len(path):
            logger.warn("Caching path does not match for f:p = {}:{}".format(len(frames), len(path)))
        # OpenCV uses BGR order of channels.
        frames = torch.index_select(frames, dim=3, index=torch.Tensor([2, 1, 0]).long())
        # FIXME: lost accuracy. It is lossy compression.
        for idx, image_path in enumerate(path):
            cv2.imwrite(image_path, frames[idx].numpy())
    else:
        with PathManager.open(path, "wb") as f:
            if format == "npz":
                # Largest, but process faster.
                np.savez(f, frames=frames.numpy())
            elif format == "npz_compressed":
                # Smaller, but more CPU computing.
                np.savez_compressed(f, frames=frames.numpy())
            else:
                raise NotImplementedError()
...
def get_ssim_filename(cfg, mode):
    return os.path.join(
        cfg.DATA.PATH_TO_DATA_DIR, "{}_ssim_{}x{}.dat".format(mode, cfg.DATA.NUM_FRAMES, cfg.DATA.SAMPLING_RATE)
    )


def get_video_cache_dir(cfg):
    return os.path.join(
        cfg.DATA.PATH_CACHE_DIR,
        "{}x{}".format(cfg.DATA.NUM_FRAMES, cfg.DATA.SAMPLING_RATE))


def get_cache_end_file(cfg, mode):
    return os.path.join(
        get_video_cache_dir(cfg),
        "END_{}".format(mode)
    )


def is_video_caching(cfg, mode):
    return cfg.DATA.CACHE and not PathManager.exists(get_cache_end_file(cfg, mode))


def is_ssim_caching(cfg, mode):
    if not cfg.DATA.SSIM_ADAPTIVE_SAMPLING.ENABLED:
        return False
    return cfg.DATA.SSIM_ADAPTIVE_SAMPLING.PRE_SSIM and not PathManager.exists(get_ssim_filename(cfg, mode))
