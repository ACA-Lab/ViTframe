...
from .ssim_eval import load_ssim_file, select_and_sort_videos, ssim_select_aux
...
class Ssv2(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        self._is_caching = self._is_ssim_caching
    ...
    def _construct_loader(self):
        ...
        for index in range(len(self._video_names)):
            if self._video_names[index] in self._path_to_videos:
                name = self._video_names[index]
                label = self._labels[index]
                videos_info_arr.append([name, label])

        self.ssim_loaded_flag = False
        path_to_ssim_file = utils.get_ssim_filename(self.cfg, self.mode)
        if self.cfg.DATA.SSIM_ADAPTIVE_SAMPLING.ENABLED and self.cfg.DATA.SSIM_ADAPTIVE_SAMPLING.PRE_SSIM and PathManager.exists(
                path_to_ssim_file):
            self.ssim_loaded_flag = True
            logger.info("{} split SSIM Data Loading ...".format(self.mode))
            vid_ssim = load_ssim_file(path_to_ssim_file)
            logger.info("{} split SSIM Selecting and Sorting ...".format(self.mode))
        ...
        for info in videos_info_arr:
            self._video_names.append(info[0])
            new_paths.append(self._path_to_videos[info[0]])
            self._labels.append(info[1])
            if self.ssim_loaded_flag:
                self._ssim_values.append(info[2])
                self._ssim_selections.append(info[3])
        ...
        self._ssim_values = list(
            chain.from_iterable([[x] * self._num_clips for x in self._ssim_values])
        )
        self._ssim_selections = list(
            chain.from_iterable([[x] * self._num_clips for x in self._ssim_selections])
        )
