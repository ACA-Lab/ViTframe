...
class Attention(nn.Module):
    ...
    def forward(self, x, cnt, signal, T):
        fw = open('./time.txt', 'a')
        B, N, C = x.shape
        ...
        if signal == 1:
          q_tmp = q[:, :, 1:, :].clone()
          q_tmp = rearrange(q_tmp, '(b t) l n m -> b t l n m',t=T)
          np.savez('/home/lizilong/TimeSformer/frame_similarity/query' + str(cnt), q_tmp.cpu().detach().numpy())
        ######
        ...
        torch.cuda.synchronize()
        elapsed_time = start.elapsed_time(end)
        fw.write('qk:' + str(elapsed_time) + '\n')
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        
        ######
        if signal == 1:
          attn_tmp = attn[:, :, 1:, 1:].clone()
          attn_tmp = rearrange(attn_tmp, '(b t) l n m -> b t l n m',t=T)
          np.savez('/home/lizilong/TimeSformer/frame_similarity/attn' + str(cnt), attn_tmp.cpu().detach().numpy())
        ######
...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W, cnt):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            res_temporal = self.drop_path(self.temporal_attn(self.temporal_norm1(xt), cnt, 0, T))
            ...
            res_spatial = self.drop_path(self.attn(self.norm1(xs), cnt, 1, T))
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        ######
        self.cnt = self.cnt + 1
        cls_tmp = x[:, 0].unsqueeze(1)
        x = x[:,1:]
        x = rearrange(x, 'b (n t) m -> b n t m',t=T)
        np.savez('/home/lizilong/TimeSformer/frame_similarity/block' + str(self.cnt), x.cpu().detach().numpy())
        x = rearrange(x, 'b n t m -> b (n t) m',t=T)
        x = torch.cat((cls_tmp, x), dim=1)
        ######
        
        ## Attention blocks
        for blk in self.blocks:
            x = blk(x, B, T, W, self.cnt)
