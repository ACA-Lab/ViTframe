...
# If the decoded pictures require to be cached.
_C.DATA.CACHE = False

# Cached video picture directory.
_C.DATA.PATH_CACHE_DIR = ""

# Caching format: "npz", "npz_compressed", "jpg"(lossy compression!)
_C.DATA.CACHE_FORMAT = "npz_compressed"

# If only run the caching process.
_C.DATA.CACHE_ONLY = False
...
_C.DATA.SSIM_ADAPTIVE_SAMPLING = CfgNode()

# Whether enabling ssim adaptive sampling.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.ENABLED = False

# Control the SSIM threshold.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.SSIM_THRESHOLD = 0.5

# SSIM Sampling fallback control.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.FALLBACK = False

# When the difference between the top2 is smaller than this value
# fallback to make full prediction.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.FALLBACK_EPSILON = 0.5

# Whether trim the tail when fallback.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.FALLBACK_TRIM_TAIL = True

# Whether fallback all frames whenever.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.FALLBACK_AUX_ALL = False

# Calculate SSIM from the middle.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.FROM_MID = True

# Calculate SSIM values on spatial crops instead of the full size frame.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.CROP_SSIM = False

# Calculate SSIM in preprocessing.
# Recommended to open it with DATA.CACHE
_C.DATA.SSIM_ADAPTIVE_SAMPLING.PRE_SSIM = False

# Limit the minimum selection number.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.SELECT_MIN = 1

# Reverse the running order.
_C.DATA.SSIM_ADAPTIVE_SAMPLING.REVERSE = True
